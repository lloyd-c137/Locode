...
**重要提示**：
- 服务器启动时会自动创建数据库文件（位于 `data/config.db`）
- 数据库用于存储 API 配置信息
- 首次启动时，数据库会自动初始化，无需手动创建

### 5. 配置 API（必须）

在使用 AI 功能前，**必须**先配置 API 信息。如果没有配置 API，使用 AI 功能时会收到错误提示。
...
**⚠️ 重要提示**：
- 数据库会在服务器启动时自动初始化（位于 `data/config.db`）
- 可以创建多个配置（如不同的 API Key 或不同的模型）
- **必须设置其中一个为默认配置**，否则 AI 功能无法使用
- 如果没有配置 API 就使用 AI 功能，系统会提示"请先配置 API 信息"
